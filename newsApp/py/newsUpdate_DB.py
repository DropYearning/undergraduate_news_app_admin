...
class Update:
    ...
    def update(self):
        # 使用pymysql连接MySQL
        conn = pymysql.connect(host=self.dbhost, port=self.dbport, user=self.dbuser, passwd=self.dbpasswd, db=self.dbname, charset=self.dbcharset)  # 创建数据库连接
        cur = conn.cursor()  # 使用 cursor()方法创建一个游标对象cursor
        updateStart = datetime.datetime.now()# 记录开始更新的时间
        ...
        for channel in self.channelDict:
            ...
            # 请求相关参数(均以str存储)
            para_channelId = self.channelDict[channel][0]  # 请求的新闻频道ID
            ...
            tableName = self.channelDict[channel][1]
            pageList = [1] # 要从API请求的页数表
            newsCount = 0  # 保存该频道更新的条数
            # 每个频道从后往前拉取50条新闻分析
            for k in pageList:
                ...
                bodys = {}
                url = self.host + self.path + '?' + myQuerys
            ...
            newsCountAll = newsCountAll + newsCount

        # 在表news_log中写入更新记录
        updateEnd = datetime.datetime.now()
        updateTime = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        usedTime = updateEnd - updateStart
        SQL_INSERT_Log = "INSERT INTO news_log (updatetime, usedtime, all_count, car_count , digit_count, demostic_count, edu_count, entertainment_count, estate_count, finance_count, game_count, international_count, internet_count, military_count,  society_count, sport_count ,tech_count) VALUES ( '%s', '%s', '%d',  '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d')" % (
        updateTime, usedTime, newsCountAll, countDict['汽车'], countDict['数码'], countDict['国内'], countDict['教育'],
        countDict['娱乐'], countDict['房地产'], countDict['财经'], countDict['游戏'], countDict['国际'], countDict['互联网'],
        countDict['军事'], countDict['社会'], countDict['体育'], countDict['科技'])
        try:
        ...
        self.logger.info(
            "本次共更新新闻[%s]条, 花费时间[%s]秒.其中[国内]新闻[%s]条,[国际]新闻[%s]条,[财经]新闻[%s]条,[互联网]新闻[%s]条,[房地产]新闻[%s]条,[汽车]新闻[%s]条,[体育]新闻[%s]条,[娱乐]新闻[%s]条,[游戏]新闻[%s]条,[教育]新闻[%s]条,[科技]新闻[%s]条,[军事]新闻[%s]条,[数码]新闻[%s]条,[社会]新闻[%s]条" % (
            newsCountAll, usedTime, countDict['国内'], countDict['国际'], countDict['财经'], countDict['互联网'],
            countDict['房地产'], countDict['汽车'], countDict['体育'], countDict['娱乐'], countDict['游戏'], countDict['教育'],
            countDict['科技'], countDict['军事'], countDict['数码'], countDict['社会']))
